...
async def extract_and_commit(
    ...
    user_content = json.dumps({
        ...
        "scenes": scenes,
        "paragraphs": [],  # TODO: pass paragraph-level data
    ...
    if conflicts:
        ...
        # Per §5.5: conflicts don't overwrite L4, but we still commit non-conflicting events

    # Filter to explicit-only events per §5.5
    explicit_events = [e for e in events if e.get("certainty") == "explicit"]

    if not explicit_events:
        # Still need to create L1 even with no events
        explicit_events = []
    ...
    # Generate scene_search_documents (simplified)
    ...
    for scene in scenes:
        search_doc = SceneSearchDocument(
            ...
            book_id=book_id,
            chapter_id=chapter_id,
            scene_id=uuid.UUID(scene["scene_id"]) if isinstance(scene.get("scene_id"), str) else (scene.get("scene_id") or uuid.uuid4()),
            ...
            search_tsv="",  # TODO: generate tsvector after Chinese tokenization
            content_hash=hash(scene.get("content", "")),
            version=1,
        )
        db.add(search_doc)
