...
from app.models import MemoryL1ChapterLedger, MemoryL2StageSummary, MemoryL3VolumeSummary, OutlineNode
...
async def generate_l2(
    ...
    # Use a simple LLM call to summarize (not a full agent - just prose)
    from app.gateway.model_gateway import stream_agent_call
    ...
    source_hash = str(hash(json.dumps(l1_data, ensure_ascii=False)))
...
async def generate_l3(
    ...
    """Generate L3 volume summary from all L2s in this volume."""
    ...
    l3 = MemoryL3VolumeSummary(
        ...
        volume_no=volume_no,
        outline_version=outline_version,
        source_hash=str(hash(json.dumps(l2_data, ensure_ascii=False))),
        status="generated",
        summary_json=summary_json or {},
        source_run_id=uuid.uuid4(),
    )
