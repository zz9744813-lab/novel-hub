...
# Build thinking tags safely to avoid encoding corruption
_THINK_OPEN = chr(60) + "think" + chr(62)        # 7 chars
_THINK_CLOSE = chr(60) + "/think" + chr(62)      # 8 chars

# Layer 1: structural markers (open_tag_pattern, close_tag_pattern)
STRUCTURE_PATTERNS = [
...
def check_leak(final_content: str) -> LeakResult:
    ...
    for i, para in enumerate(paragraphs):
        ...
        for open_tag, close_tag in STRUCTURE_PATTERNS:
            ...
            # Also check for close tag without open (orphaned)
            if close_tag and re.search(close_tag, para_stripped):
                found.append({
                    "paragraph_id": para_id,
                    "classification": "reasoning_leak",
                    "confidence": 0.85,
