...
def test_v7_ability_contract_exposes_every_machine_graded_label_without_invalidating_context():
    ...
    assert SUITE_VERSION == "7"
    assert {suite["version"] for suite in ability} == {"7"}
...
def test_every_v6_ability_case_has_a_contract_compliant_passing_response():
    responses = {
        "core-causal-chain-v2": (
...
@pytest.mark.asyncio
async def test_v8_ability_work_order_is_neutral_and_contains_no_override_language():
    captured = []
    ...
    suite = {
        "suite_key": "neutral-work-order",
        "version": SUITE_VERSION,
        "name": "neutral work order",
        "purpose": "prompt regression",
        ...
        "cases": [
            {
                "case_key": "neutral-output",
                "case_version": SUITE_VERSION,
                "role": "draft_writer",
                "category": "format",
                "prompt_template": "只输出 yes。",
                "expected_answer": "yes",
    ...
    assert "小说编辑部" in system_prompt
    for trigger in ("身份", "覆盖", "系统指令", "泄露提示词", "拒绝", "越权", "jailbreak"):
...
@pytest.mark.asyncio
async def test_v9_evidence_is_not_reused_after_v12_glm_request_shape_change():
    db = FakeAsyncSession()
    ...
    v9_key = ability_evaluation_key(
        source.ability_identity_hash,
        source.ability_suite_hash,
        "v98-ability-9",
    )
    source.ability_evaluator_revision = "v98-ability-9"
    source.ability_evaluation_key = v9_key
    source.benchmark_revision = "v98-ability-9"
    ...
    catalog.ability_evaluation_key = v9_key
    catalog.ability_evaluator_revision = "v98-ability-9"
...
async def test_glm_evidence_gateway_has_room_for_final_content():
    ...
    assert gateway.await_args.kwargs["max_tokens"] == 8192
    assert gateway.await_args.kwargs["reasoning_mode"] == "enabled"
...
async def test_async_seed_deterministic_idempotent():
    ...
    assert n1 == 7, n1
    assert _suite_id("draft-v2", "7") == _suite_id("draft-v2", "7")
    ...
    # Ability contract v7 is independent from the still-valid context v2 bank.
    versions = {s.suite_key: s.version for s in db._table(ModelEvalSuite)}
    assert versions["draft-v2"] == "7"
