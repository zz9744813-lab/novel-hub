...
能力题库与上下文阶梯独立版本化：只修订能力题目不会让已经测得的 128K 上下文证据失效。资格题中的机器判分枚举必须在题面公开；每个强制用例都要达到本套件下限，不能靠其他题的高平均分掩盖单项失败。
...
显式生产发布会先保留已经合格的现有绑定。若某个绑定不合格，则只在本次已经评测且上下文足够的配置模型中选择分数最高的替代者，并把变更写入发布报告；不会为了路由临时启动一轮新能力测试。
