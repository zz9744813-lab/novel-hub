...
服务器只接受 `origin/main` 上的提交。流程依次执行镜像拉取、独立版本目录、目标机 Compose 配置展开校验、PostgreSQL 就绪检查与迁移前自定义格式逻辑备份、镜像构建、Alembic 升级、生产包校验、原子切换、容器启动和 `/health/ready` 验收。备份保存在 root 专用的 `/srv/novelforge/shared/data/backups/`；Compose 配置无效或备份失败时都不会开始迁移。应用启动失败时 `current` 自动切回上一个版本。数据库不会自动降级或自动恢复备份，因此迁移仍须保持前后版本可兼容，恢复操作必须在提供商控制台人工确认后进行。
