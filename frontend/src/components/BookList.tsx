    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="panel flex flex-col items-center py-16 text-text-tertiary">
          ...
          <p className="text-xs">
            {search ? "没有匹配的项目" : "暂无项目，点击「新建」创建"}
          </p>
        </div>
      ) : (
        <div className="space-y-1">
          {filtered.map((b) => {
            const id = b.book_id || (b as any).id;
            const active = selectedBookId === id;
            const progress = b.target_chapters
              ? (b.finalized_chapters || 0) / b.target_chapters
              : 0;
            return (
              <div
                key={id}
                ...
                className={clsx("row-item cursor-pointer", active && "active")}
              >
                    ...
                    {b.target_chapters ? (
                      <div className="progress-bar flex-1 max-w-[80px]">
                        <div
                          className={clsx(
                            "progress-fill",
                            progress >= 1 ? "bg-success" : "bg-brand"
                          )}
