import { BookOpen, GitGraph, FileText, Brain, AlertTriangle, Plus, PenTool, Search } from "lucide-react";
  ...
  return (
    <aside className="w-52 bg-bg-panel border-r border-border flex flex-col shrink-0">
      ...
      <div className="h-11 flex items-center px-4 border-b border-border">
        ...
        <span className="ml-auto text-2xs text-text-disabled font-mono">v7.3</span>
      </div>
      ...
      <nav className="flex-1 p-2 space-y-px">
        {tabs.map((t) => {
          ...
          const disabled = t.id !== "overview" && !selectedBookId;
          return (
            <button
              key={t.id}
