...
export function LibraryEmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div className="panel-elevated rounded-card flex flex-col items-center py-20 text-center">
      <div className="mb-4 grid h-14 w-14 place-items-center rounded-card bg-brand-muted border border-brand/20 shadow-glow">
        <BookPlus size={24} className="text-brand-accent" />
      </div>
      <p className="text-emphasis text-text-primary" style={{ fontWeight: 510 }}>
        书架还是空的
      </p>
      <p className="text-body text-text-tertiary mt-1 max-w-sm leading-relaxed">
